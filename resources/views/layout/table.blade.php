    ...
    <tbody>
        ...
        <td>
          @if($sold->one_stock->type  == 0)
            {!!DNS1D::getBarcodeSVG("$sold->stock_id", 'C128' , 1,24 , 'dark', false)!!}
            @else
        ...
        @if(Request::segment(1) != 'sellers')
      <td class="bg-danger text-white border border-white" onclick="undo(`{{$sold->id}}`)"><i class="fas fa-undo-alt"></i></td>
        @endif
      </tr>
      @endforeach
    </tbody>
